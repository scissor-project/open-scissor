...
name = __MARIADB_PRELUDE_NAME__

# Username to be used to connect the database.
user = __MARIADB_PRELUDE_USER__

# Password used to connect the database.
pass = __MARIADB_PRELUDE_PASS__
...
[Unit]
Description=Prelude profile registrator

[Service]
ExecStart=/usr/bin/prelude-admin registration-server prelude-manager --no-confirm --keepalive --passwd-file /etc/prelude/prelude-registrator.conf
