# Felipe: reconf: BM assumes that whatever is talking to it (to the BM daemon) keeps track of changes to a running composition. That is, a reconfiguration command includes XML that *only* has changes to the running composition, *not* a new composition (BM does *not* do a diff between an existing composition and a new one, this is up to whatever is using BM, e.g., a GUI).
