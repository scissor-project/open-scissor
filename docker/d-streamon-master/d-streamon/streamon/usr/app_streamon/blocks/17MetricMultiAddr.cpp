...
namespace bm
{
	class MetricMultiAddr : public Metric_Block
	{
		public:
			/*
			 * costruttore
			 */
			MetricMultiAddr(const std::string &name, bool): Metric_Block(name, false)
	                {
			  /*
			  id_ingate = register_input_gate("in_MetricMultiAddr");
			  id_outgate = register_output_gate("out_MetricMultiAddr");
			  */
			}

			MetricMultiAddr(const MetricMultiAddr &)			 = delete;				
			MetricMultiAddr& operator=(const MetricMultiAddr &)    = delete;		
			MetricMultiAddr(MetricMultiAddr &&)				 = delete;					
			MetricMultiAddr& operator=(MetricMultiAddr &&)		 = delete;			

			/*
			 * distruttore
			 */
			virtual ~MetricMultiAddr()
			{}
  ...
  BlockFactoryRegistration<MetricMultiAddr> __MultiAddr_registration__("MetricMultiAddr");


  // REGISTER_BLOCK(MetricMultiAddr,"MetricMultiAddr");
}
