	    ...
	    class Exporter : public Block
	    {
	        ...
	        virtual void _receive_msg(std::shared_ptr<const Msg>&& m, int /*id_ingate*/)
	        {
	            ...
	            auto publishMap = Tags->zmq_publish;
							...
							//std::cout << "Load result: " << result.description() << ", doc name: " << doc << std::endl;

							//std::cout << "Load result: " << result.description() << ", mesh name: " << doc.child("composition").attribute("probe_id").value() << std::endl;

							auto probe_id = doc.child("composition").attribute("probe_id").value();

	          	for (std::map<std::string,std::string>::iterator it=publishMap.begin(); it!=publishMap.end(); ++it)
	          	{

						std::cout << probe_id << " " << it->first << " => " << it->second << '\n';

						s_sendmore (publisher, probe_id);
