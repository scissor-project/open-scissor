...
Variables = Config.getElementsByTagName('variable')
ProbeID = sys.argv[2]
...
print"[ADD&SAVE_VARIABLES]"

var = "//Global Variables open\n\n"
text = ""
var2 = ""
cout = ""
...
for Variable in Variables:
    ...
    VariableInValue = Variable.getAttribute('initial_value')
    VariableComment = Variable.getAttribute('comment')

    print "VariableName = " + VariableName + "\n"
    print "VariableType = " + VariableType + "\n"
    print "VariableInValue = " + VariableInValue + "\n"
    print "VariableComment = " + VariableComment + "\n"

    var = var + "// " + VariableComment + "\n" + VariableType + " " + VariableName.replace("$", "var_") + " = " + VariableInValue + ";\n\n"

    #text = text + "# " + VariableComment + "\n" + VariableName + "\n" + VariableType + "\n" + VariableInValue + "\n\n"
    text = text + "# " + "(" + VariableType + ") " + VariableComment + "\n" + VariableName.replace("$", "") + ": " + VariableInValue + "\n\n"

    if VariableType == "int":
        var2 = var2 + "::" + VariableName.replace("$", "var_") + " = " + "std::stoi(get_value(values["+str(j)+"]))" + ";\n"
        #var2 = var2 + "::" + VariableName.replace("$", "var_") + " = " + "get_value(values["+str(j)+"])" + ";\n"
    elif VariableType == "bool":
        var2 = var2 + "::" + VariableName.replace("$", "var_") + " = " + "to_bool(get_value(values["+str(j)+"]))" + ";\n"

    j = j + 3

    cout = cout + "std::cout << ::" +  VariableName.replace("$", "var_") + " << std::endl;"
...
MonstreamLibRows.append(var+"//Global Variables closed\n\n")

#MonstreamLibRows.append("HashTable table_id;\n")

#MonstreamLibRows.append("HashTable table_mac;\n")

MonstreamLibRows.append("std::unordered_set<std::string> ip_wlist;\n")

MonstreamLibRows.append("std::unordered_set<std::string> ip_blist;\n")
...
MonstreamLibRows.append(to_bool_func)

get_value_func = '''\n std::string get_value(std::string str)
{
     auto const pos=str.find_last_of(':');
     std::string leaf=str.substr(pos+1);
...
MonstreamLibRows.append(get_value_func)
...
MonstreamLibRows.append(refresh_func)
